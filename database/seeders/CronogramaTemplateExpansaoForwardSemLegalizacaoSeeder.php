...
/**
 * TEMPLATE 2 — EXPANSÃO — Início > Fim (sem Legalização).
 *
 * Modelado a partir das fórmulas da planilha PMO oficial (aba EXPANSÃO, linha 4).
 * Idêntico ao Template 1, mas sem a fase PRAZO_LEGAL — POSSE depende
 * diretamente do fim de ORCAMENTOS.
 *
 * Span total: 187 dias offset (12 dias a mais que T1, pois sem legalização
 * a Posse passa a depender do fim dos Orçamentos, que terminam mais tarde
 * que o Prazo Legal — este corre em paralelo com o Executivo).
 */
class CronogramaTemplateExpansaoForwardSemLegalizacaoSeeder extends Seeder
{
    public function run(): void
    {
        $template = CronogramaTemplate::updateOrCreate(
            ['nome' => 'Expansão — Início > Fim (sem Legalização)'],
            [
                'tipo_obra' => TipoObraCronograma::EXPANSAO->value,
                'ancora_campo' => 'projeto.cad_plan_inicio',
                'ativo' => true,
                'observacoes' => 'Template progressivo ancorado em INICIO_PROJETO. Sem legalização: PRAZO_LEGAL é omitida e POSSE depende do fim de ORCAMENTOS. Span total = 187 dias.',
            ]
        );
