...
@endphp
<div wire:key="ep-si-{{ $item->id }}" x-data="{ open: false }" style="margin-bottom:2px;">
    ...
    <div style="display:flex;align-items:center;gap:4px;padding-left:{{ $paddingLeft }}px;">
        ...
        <input type="checkbox"
               ...
               style="width:11px;height:11px;cursor:pointer;flex-shrink:0;">
        <input type="text"
               value="{{ $item->titulo }}"
               wire:change="salvarTituloSubitem({{ $item->id }}, $event.target.value)"
               class="{{ $item->recebido ? 'cr-subitem-done' : '' }}"
        ...
        @endif
        <button type="button"
                wire:click="removerSubitem({{ $item->id }})"
    ...
    <div x-show="open" x-cloak
        ...
        @forelse($item->dependencias as $dep)
            <div style="display:grid;grid-template-columns:1fr 72px 42px 16px;gap:2px;align-items:center;margin-bottom:2px;"
                ...
                <button type="button"
                        wire:click="removerDependenciaSubitem({{ $dep->id }})"
        ...
        <button type="button"
                wire:click="adicionarDependenciaSubitem({{ $item->id }})"
        ...
        <div style="margin-top:6px;border-top:1px solid var(--vo-border-light);padding-top:4px;">
            ...
            @if($expandindoFilhosDeItemId === $item->id)
                <div style="display:flex;gap:3px;margin-top:3px;">
                    ...
                    <button type="button"
                            wire:click="adicionarFilhoItem({{ $item->id }})"
