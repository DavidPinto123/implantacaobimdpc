...
<style>
    ...
    .obras-compact .fi-ta-header-toolbar {
        display: flex;
        gap: 10px;
        padding: 10px 16px;
        background: var(--vo-bg);
    ...
    .obras-compact .fi-input-wrp,
    .obras-compact .fi-select-input,
    .obras-compact .fi-input {
    ...
    .obras-compact .fi-ta-cell .fi-input-wrp {
        min-height: 0 !important;
        height: auto !important;
    }

    .obras-compact .fi-ta-cell .fi-input,
    .obras-compact .fi-ta-cell .fi-select-input,
    .obras-compact .fi-ta-cell input[type="text"],
    .obras-compact .fi-ta-cell input[type="date"],
    .obras-compact .fi-ta-cell select {
        height: auto !important;
        min-height: 0 !important;
    ...
    .obras-compact .fi-ta-table {
        width: 100%;
        border-collapse: collapse;
    }

    .obras-compact .fi-ta-header-cell {
    ...
    .obras-compact tbody tr:hover {
        background: var(--vo-bg-subtle);
    }

    .obras-compact .fi-ta-filter-indicators {
        padding: 8px 16px;
        border-bottom: 1px solid var(--vo-border-light);
        background: var(--vo-bg);
    }
...
>
                    ...
                    @if ($isGlobalSearchVisible || $hasFiltersTrigger || $hasColumnManagerDropdown)
                        <div>
                            ...
                            @if ($hasFiltersTrigger || $hasColumnManagerDropdown)
                                @php
                                    $selectFilters = \App\Filament\Resources\Obras\Tables\ObrasColumnFilters::getSelectFilters();
                                    $dateRangeColumns = \App\Filament\Resources\Obras\Tables\ObrasColumnFilters::getDateRangeColumns();

                                    // Build filter groups matching column groups
                                    $filterGroups = [];
                                    foreach ($columns as $col) {
                                        $colName = $col->getName();
                                        $filterType = \App\Filament\Resources\Obras\Tables\ObrasColumnFilters::getFilterType($colName);
                                        if (! $filterType) continue;

                                        $groupLabel = $col->getGroup()?->getLabel() ?? 'Outros';
                                        if (!isset($filterGroups[$groupLabel])) {
                                            $filterGroups[$groupLabel] = [];
                                        }
                                        $filterGroups[$groupLabel][] = [
                                            'name' => $colName,
                                            'label' => strip_tags($col->getLabel()),
                                            'type' => $filterType,
                                            'options' => $filterType === 'select' ? \App\Filament\Resources\Obras\Tables\ObrasColumnFilters::getSelectOptions($colName) : [],
                                        ];
                                    }
                                @endphp

                                <div x-data="{ filtersModalOpen: false }" class="fi-ta-filters-custom">
                                    <button type="button"
                                            @click="filtersModalOpen = true"
                                            class="fi-icon-btn fi-icon-btn-size-md"
                                            title="Filtros"
                                    >
                                    ...
                                    <template x-teleport="body">
                                        <div x-show="filtersModalOpen"
                                             ...
                                             @click.self="filtersModalOpen = false"
                                             @keydown.escape.window="filtersModalOpen = false"
                                             style="display:none;">
                                            ...
                                            <div class="fi-colmgr-modal" @click.stop>
                                                ...
                                                <div class="fi-colmgr-head">
                                                    ...
                                                    <div class="fi-colmgr-head-actions">
                                                        <button type="button"
                                                                @click="$wire.call('resetTableFiltersForm'); filtersModalOpen = false;"
                                                                class="fi-colmgr-reset-btn">
                                                            Limpar Filtros
                                                        </button>
                                                        <button type="button" @click="filtersModalOpen = false" class="fi-colmgr-close-btn">
                                                ...
                                                <div class="fi-colmgr-body">
                                                    @foreach ($filterGroups as $groupLabel => $groupFilters)
                                                                        ...
                                                                        <div class="fi-filtmgr-control">
                                                                            @if ($filter['type'] === 'select')
                                                                                <div
                                                                                    ...
                                                                                    x-data="{
                                                                                        ...
                                                                                        get values() {
                                                                                            const values = $wire.tableFilters?.['{{ $filter['name'] }}']?.values;
                                                                                        ...
                                                                                        toggleValue(value) {
                                                                                            ...
                                                                                            $wire.set('tableFilters.{{ $filter['name'] }}.values', values);
                                                                                        },
                                                                                        clearValues() {
                                                                                            $wire.set('tableFilters.{{ $filter['name'] }}.values', []);
                                                                                        },
                                                                                        get selectedLabels() {
                                                                            ...
                                                                            @elseif ($filter['type'] === 'date_range')
                                                                                <div class="fi-filtmgr-date-group">
                                                                                    <input
                                                                                        type="date"
                                                                                        wire:model.live.debounce.800ms="tableFilters.{{ $filter['name'] }}.from"
                                                                                    ...
                                                                                    <input
                                                                                        type="date"
                                                                                        wire:model.live.debounce.800ms="tableFilters.{{ $filter['name'] }}.until"
                                                                                        class="fi-filtmgr-date"
                                                                                        placeholder="AtÃ©"
                                                                                    />
                                                                                </div>
                                                                            @elseif ($filter['type'] === 'text')
                                                                                <input
                                                                                    type="text"
                                                                                    wire:model.live.debounce.300ms="tableFilters.{{ $filter['name'] }}.value"
                                                                                    class="fi-filtmgr-input"
                                                ...
                                                <div class="fi-colmgr-foot">
                                                    <button type="button" @click="filtersModalOpen = false"
                                    ...
                                    @php
                                        ...
                                        foreach ($this->getTable()->getFilters() as $filter) {
                                            if ($filter instanceof \Filament\Tables\Filters\SelectFilter) {
                                                $headerFilterOptions[$filter->getName()] = $filter->getOptions();
                                            }
                                        }
                                    @endphp
                                        ...
                                        @else
                                            @php
                                                ...
                                                $columnHeaderTooltipAttribute = ($columnHeaderTooltip instanceof \Illuminate\Contracts\Support\Htmlable)
                                                    ? 'x-tooltip.html'
                                                    : 'x-tooltip';
                                                $columnFilterType = \App\Filament\Resources\Obras\Tables\ObrasColumnFilters::getFilterType($columnName);
                                                $columnHasFilter = $columnFilterType !== null;
                                            @endphp
                                            ...
                                            >
                                                <div class="fi-ta-header-cell-inner">
                                                    @if ($isColumnSortable)
                                                        <span
                                                    ...
                                                    @if ($columnHasFilter)
                                                        <div
                                                            x-data="{
                                                                ...
                                                                panelStyle: {},
                                                                get isActive() {
                                                                    @if ($columnFilterType === 'select' || $columnFilterType === 'text')
                                                                        @if ($columnFilterType === 'select')
                                                                            const values = $wire.tableFilters?.['{{ $columnName }}']?.values ?? [];
                                                                            const value = $wire.tableFilters?.['{{ $columnName }}']?.value;
                                                                            return Array.isArray(values) ? values.length > 0 : (value !== undefined && value !== null && value !== '');
                                                                        @else
                                                                            const v = $wire.tableFilters?.['{{ $columnName }}']?.value;
                                                                            return v !== undefined && v !== null && v !== '';
                                                                        @endif
                                                                    @else
                                                                        const f = $wire.tableFilters?.['{{ $columnName }}'];
                                                                        return (f?.from && f.from !== '') || (f?.until && f.until !== '');
                                                                    @endif
                                                                },
                                                                get selectedValues() {
                                                                    const values = $wire.tableFilters?.['{{ $columnName }}']?.values;
                                                                ...
                                                                toggleSelectValue(value) {
                                                                    ...
                                                                    $wire.set('tableFilters.{{ $columnName }}.values', values);
                                                                },
                                                                toggle() {
                                                                    this.open = !this.open;
                                                                    if (this.open) {
                                                            ...
                                                            class="fi-header-filter-wrapper"
                                                        >
                                                            <button
                                                                type="button"
                                                                x-ref="btn"
                                                                @click.stop="toggle()"
                                                                class="fi-header-filter-btn"
                                                                :class="{ 'active': isActive }"
                                                                title="Filtrar"
                                                            >
                                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="fi-header-filter-icon">
                                                                    <path fill-rule="evenodd" d="M2.628 1.601C5.028 1.206 7.49 1 10 1s4.973.206 7.372.601a.75.75 0 0 1 .628.74v2.288a2.25 2.25 0 0 1-.659 1.59l-4.682 4.683a2.25 2.25 0 0 0-.659 1.59v3.037c0 .684-.31 1.33-.844 1.757l-1.937 1.55A.75.75 0 0 1 8 18.25v-5.757a2.25 2.25 0 0 0-.659-1.591L2.659 6.22A2.25 2.25 0 0 1 2 4.629V2.34a.75.75 0 0 1 .628-.74Z" clip-rule="evenodd" />
                                                                </svg>
                                                            </button>
                                                            ...
                                                            <template x-teleport="body">
                                                                <div
                                                                    ...
                                                                    x-transition.opacity.duration.150ms
                                                                    @click.outside="open = false"
                                                                    class="fi-header-filter-panel"
                                                                    :style="panelStyle"
                                                                    x-cloak
                                                                    @click.stop
                                                                >
                                                                    @if ($columnFilterType === 'select' && isset($headerFilterOptions[$columnName]))
                                                                        <div class="fi-header-filter-options">
                                                                            <button
                                                                                type="button"
                                                                                class="fi-header-filter-option"
                                                                                :class="{ 'active': selectedValues.length === 0 }"
                                                                                @click="$wire.set('tableFilters.{{ $columnName }}.values', [])"
                                                                            >Todos</button>
                                                                            @foreach ($headerFilterOptions[$columnName] as $optValue => $optLabel)
                                                                                <button
                                                                                    type="button"
                                                                                    class="fi-header-filter-option"
                                                                                    :class="{ 'active': hasValue(@js($optValue)) }"
                                                                                    @click="toggleSelectValue(@js($optValue))"
                                                                                >{{ $optLabel }}</button>
                                                                            @endforeach
                                                                            <button
                                                                                type="button"
                                                                                class="fi-header-filter-clear"
                                                                                @click="open = false"
                                                                            >
                                                                                Fechar
                                                                            </button>
                                                                        </div>
                                                                    @elseif ($columnFilterType === 'date_range')
                                                                        <label class="fi-header-filter-label">De</label>
                                                                        <input
                                                                            type="date"
                                                                            wire:model.live.debounce.800ms="tableFilters.{{ $columnName }}.from"
                                                                            class="fi-header-filter-date-input"
                                                                        />
                                                                        <label class="fi-header-filter-label">Até</label>
                                                                        <input
                                                                            type="date"
                                                                            wire:model.live.debounce.800ms="tableFilters.{{ $columnName }}.until"
                                                                            class="fi-header-filter-date-input"
                                                                        />
                                                                    @elseif ($columnFilterType === 'text')
                                                                        <input
                                                                            type="text"
                                                                            wire:model.live.debounce.300ms="tableFilters.{{ $columnName }}.value"
                                                                            class="fi-header-filter-text-input"
                                                                        />
                                                                    @endif

                                                                    @if ($columnFilterType === 'date_range')
                                                                        <button
                                                                            type="button"
                                                                            class="fi-header-filter-clear"
                                                                            @click="
                                                                                $wire.set('tableFilters.{{ $columnName }}', { from: null, until: null });
                                                                                open = false;
                                                                            "
                                                                        >
                                                                            Limpar
                                                                    ...
                                                                    @elseif ($columnFilterType === 'text')
                                                                        <button
                                                                            ...
                                                                            class="fi-header-filter-clear"
                                                                            @click="
                                                                                $wire.set('tableFilters.{{ $columnName }}.value', null);
                                                                                open = false;
                                                                            "
                                                                        >
                                                                            Limpar
                                                ...
                                                <div class="fi-ta-resize-handle" onmousedown="window.__startColResize(event, '{{ $columnName }}')"></div>
                                            </th>
                                        @endif
                                    @endforeach
                                        ...
                                        @foreach ($records as $record)
                                            ...
                                            @if (! $isGroupsOnly)
                                                <tr
                                                    ...
                                                    x-on:click="
                                                        if ($event.target.closest('a, button, input, select, textarea, .fi-ta-actions, .fi-ta-selection-cell')) return;
                                                        $el.closest('tbody').querySelectorAll('.fi-ta-row-highlighted').forEach(r => { if (r !== $el) r.classList.remove('fi-ta-row-highlighted') });
                                                        $el.classList.toggle('fi-ta-row-highlighted');
                                                    "
                                                    @class([
                                                        'fi-ta-row',
    ...
    <script>
    (function() {
        if (window.__fiColResizeInit) return;
        window.__fiColResizeInit = true;

        var SK = 'fi_col_widths_' + window.location.pathname;
        ...
        function applyWidths() {
            ...
            for (var col in widths) {
                var kebab = col.replace(/[._]/g, '-');
                var th = document.querySelector('.fi-ta-header-cell-' + kebab);
                if (th) {
                    th.style.width = widths[col] + 'px';
                    th.style.minWidth = widths[col] + 'px';
                    th.style.maxWidth = widths[col] + 'px';
                }
            }
        }

        window.__startColResize = function(e, colName) {
            e.preventDefault();
            e.stopPropagation();

            var kebab = colName.replace(/[._]/g, '-');
            var th = document.querySelector('.fi-ta-header-cell-' + kebab);
            if (!th) return;
            ...
            function onUp(ev) {
                ...
                setColWidth(th, finalW);
                var widths = load();
                widths[colName] = Math.round(finalW);
                localStorage.setItem(SK, JSON.stringify(widths));
            }
        ...
        // Aplicar larguras salvas quando DOM estiver pronto
        ...
        // Reaplicar após Livewire atualizar
        if (window.Livewire) {
